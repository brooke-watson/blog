...
Beyoncé Knowles-Carter is an icon who needs no introduction. In her 36 years, she has become the [highest-paid](http://www.mtv.co.uk/beyonce/news/beyonce-named-highestearning-black-artist-of-all-time) black artist of all time, the most-nominated woman in [Grammy history](http://www.billboard.com/articles/events/grammys-2015/6363698/beyonce-most-grammy-nominated-woman), and the top selling artist of the last decade. _Time_ put her on the [cover](http://www.bbc.com/news/entertainment-arts-27154568) of its 100 most influential people issue. 
...
g1 <- yonces %>% 
  ...
  annotate("label", x = 1965, y = 30, label = "Beyoncé Knowles", family = "Raleway") +
    theme_raleway() + 
    theme_fancy() + 
...
g2 <-  yonces %>% 
    ...
    xlim(1995, 2015) + 
    ggtitle("Yoncés through the years") +
...
g2 <- yonces %>%  
  select(-sex, -prop) %>% 
   ggplot(., aes(x = year, y = n)) +
    geom_line(aes(col = name)) + 
    xlim(1995, 2015) + 
    ggtitle("Yoncés through the years, 1995-2015") +
    # labs(subtitle = "Children born in the US, 1995-2015") + 
    scale_color_manual(values = c("#e41a1c", "#377eb8", "#4daf4a", "#984ea3", "#ff7f00", "#ffff33", "#a65628", "#f781bf", "#999999", "#999999")) +
    theme_raleway() + 
    theme_fancy() +   
  geom_point(data = g1annot, aes(x = year, y = n, col = NULL, label = album)) + 
        ggrepel::geom_label_repel(data = g1annot, aes(x = year, y = n, label = album), 
                                 family = "Raleway") + 
    labs(caption = wrap90("Naming a child Beyoncé reached it's peak in 2001, around the release of the Destiny's Child album 'Survivor'. This is the same year that saw the first cohort of Alternative Yoncés, such as Ceyonce and Keyonce."), y = "Annual Yoncés born in the US")
...
Perhaps in 2001, it was easier to think of "*Beyoncé*" as merely a beautiful, unique name that was growing in familiarity on local radios - much like the names [Belcardis](https://en.wikipedia.org/wiki/Cardi_B) or  [Normani](https://en.wikipedia.org/wiki/Normani) may be today. Certainly for the hundreds of parents of Keyonce, Veyonce and Deyonce, the name might've been little more than a nice set of syllables, made even more unique for their own newborns with a different leading consonant. 
...
unzip("nbs.zip", exdir = here::here("lol/"))

# get list of files and lapply read_csv over them  
files <- paste0("namesbystate/", list.files("namesbystate", pattern = "*.TXT"))

# read all the files in as comma-separated values 
bystate <- lapply(files, function(x) {
    readr::read_csv(x, col_names = c("state", "sex", "year", "name", "n"))})
df <- bind_rows(bystate)

# filter out just the yonces by state 
ybs <- df %>% 
    filter(str_detect(name, "yonce") == TRUE) 

tabyl(ybs, name) # they're all Beyonce. go figure. 

tabyl(ybs, year) # and they're all between 1999 and 2013. 
```
...
ybs <- full_join(yonces_by_state, big, by = c("state", "year"))
