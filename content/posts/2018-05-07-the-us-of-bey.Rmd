...
![](/figs/2018-05-06-us-of-bey/USofBey.gif)
...
Perhaps in 2001, it was easier to think of "*Beyoncé*" as merely a beautiful, unique name that was growing in familiarity on local radios - much like the names [Belcardis](https://en.wikipedia.org/wiki/Cardi_B) or  [Normani](https://en.wikipedia.org/wiki/Normani) may be today.

# Alternative Yoncés

By 2001, names that *rhyme* with Beyoncé were also picking up steam. They get massively overshadowed in the larger graph, but there are quite a few of these Alternate Yoncés, and the cohorts persist over several years before puttering out around 2007. But which ones are most popular? 
```{r, eval}
library(viridis)
...
gganimate::gganimate(g1b, "../figs/2018-05-06-us-of-bey/alt-yonces.gif", ani.width = 800, ani.height = 600)
...
Certainly for the hundreds of parents of Keyonce, Veyonce and Deyonce, the name might've been little more than a nice set of syllables that could be made even more unique for their own newborns with a different leading consonant. 
...
```{r}
unique(ybs$name) # they're all Beyonce. go figure. 

sort(unique(ybs$year)) # and they're all between 1999 and 2013. 
```

Unsurprisingly, all of the less common variants - Keyonce, Deyonce, and Breyonce - are hidden in the state-level data to protect privacy. However, there are enough Beyoncés left to pick up some patterns across states.


```{r}
# get a data frame of all the state-year combos to fill in missing NAs
# starting in 1995 and ending in 2015 to match with the earlier data 
big <- df %>% 
    select(state, year) %>% 
    unique() %>% 
...
gganimate::gganimate(g, "../figs/2018-05-06-us-of-bey/USofBey.gif", ani.width = 560, ani.height = 360)
```
